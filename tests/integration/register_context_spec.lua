...
test.describe("tracked unnamed register context", function()
  ...
  test.it("formats a characterwise yank using its inclusive source range", function()
    ...
    local context_result = context.capture("yank")
    test.equal(context_result:is_ok(), true)
    local value = assert(context_result.value)
    test.equal(value.kind, "file")
    test.equal(value.path, ".toss-register-fixture")
  ...
  test.it("sends non-file register text literally and preserves it for paste", function()
    ...
    local context_result = context.capture("yank")
    test.equal(context_result:is_ok(), true)
    test.equal(context_result.value.kind, "text")
